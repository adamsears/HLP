...
#time top
my $rv = $dbh->do("INSERT into user_time_toptotal select user,SUM(nummaps+numreduces) as total_tasks,round(SUM(finishtime-launchtime),2)/3600000 as total_time from b_job_info group by user order by total_time desc");
print "user_time_toptotal inserted! $rv \n";
#task_top
my $rv = $dbh->do("INSERT into user_task_toptotal select user,SUM(nummaps+numreduces) as total_tasks,round(SUM(finishtime-launchtime),2)/3600000 as total_time from b_job_info group by user order by total_tasks desc");
...
my $rv = $dbh->do("insert into main_jymr values ((SELECT count(*) FROM b_job_info),(SELECT count(*) FROM b_job_info where stauts <> 'SUCCEEDED'),(SELECT count(*) FROM b_job_info,(SELECT appid FROM l_union_unique WHERE pmemused >= pmemset)as ta where ta.appid = b_job_info.appid and b_job_info.stauts <> 'SUCCEEDED'))");
