...
##basic table create
my $rv = $dbh->do("CREATE TABLE b_ctn_mem  (ctn_id varchar(40) NOT NULL PRIMARY KEY ,pmemused float ,pmemset float,vmemused float,vmemset float ,date DATE ,time TIME,appid varchar(20),INDEX `appid` (`appid`) USING HASH)");
...
my $rv = $dbh->do("CREATE TABLE `l_union_queue` (`appid`  varchar(255),`task_id`  varchar(40),`attempt_id`  varchar(40),`ctn_id`  varchar(40),`tasktype`  varchar(6),`hostname`  varchar(40),`taskstatus`  varchar(20),`attemptstarttime`  bigint(20) NULL DEFAULT NULL ,`attemptfinishtime`  bigint(20) NULL DEFAULT NULL ,`pmemused`  float NULL DEFAULT NULL ,`pmemset`  float NULL DEFAULT NULL ,`vmemused`  float NULL DEFAULT NULL ,`vmemset`  float NULL DEFAULT NULL ,`date`  date NULL DEFAULT NULL ,`time`  time NULL DEFAULT NULL ,`jobname`  varchar(255),`user`  varchar(255),`queue`  varchar(50),`stauts`  varchar(20),PRIMARY KEY (`ctn_id`))");
print "l_union_queue created! $rv \n";

my $rv = $dbh->do("CREATE TABLE `task_sample_1min` (timestamp datetime,user varchar(255),queue varchar(20),jobname varchar(255),pmemused float,pmemset float,attemptstarttime bigint(20),attemptfinishtime bigint(20),INDEX `stamp` (`timestamp`) USING HASH)");
...
my $rv = $dbh->do("CREATE TABLE mem_time_6h (timestamp int,asr_dictt float,asr_embed float,asr_embedrecog float,asr_trans float,call_edu float,call_music float,eht float,hw_tts float,nlp float,sre float,INDEX `index_time` (`timestamp`) USING HASH)");
print "mem_time_5s created! $rv \n";
