...
#    	}
    ...
    if($type eq "MAP_ATTEMPT_STARTED"){
    	...
    	print "$taskid"." $attemptId\t"."STARTED\t"."$containerId\t$taskType\t$trackerName\t$startTime\n";
    ...
    if($type eq "MAP_ATTEMPT_FINISHED"){
    	...
    	print "$taskid"." $attemptId\t"."FINISHED\t"."$taskStatus\t$finishTime\n";
	...
	#   	my $error = $json_obj->{'event'}->{'org.apache.hadoop.mapreduce.jobhistory.TaskAttemptUnsuccessfulCompletion'}->{'error'};
    	...
    	print "$taskid"." $attemptId\t"."KILLED\t"."$status\t$finishTime\n";
...
#    	my $error = $json_obj->{'event'}->{'org.apache.hadoop.mapreduce.jobhistory.TaskAttemptUnsuccessfulCompletion'}->{'error'};
    	...
    	print "$taskid"." $attemptId\t"."FAILED\t"."$status\t$finishTime\n";
